...
def prepare_features(df):
    """
    Prepare the EXACT 20 features used by the trained CatBoost model.

    Feature order is deliberately fixed to match the training script.
    `observed_intervention` is retained because the model is
    action-conditioned; it is overwritten with each candidate action
    during policy evaluation.
    """
    missing = [c for c in MODEL_FEATURES if c not in df.columns]
    if missing:
        raise RuntimeError(
            "Evaluation dataset is missing model features:\n"
            + "\n".join(f"  - {c}" for c in missing)
        )

    X = df[MODEL_FEATURES].copy()

    # Match training-time categorical representation exactly.
...
def main():
    ...
    # The AI model needs the EXACT training feature set.
    # Do not omit numeric context features: the trained CatBoost model
    # expects 20 features in the training order.
    model_columns = list(MODEL_FEATURES)

    # Add IDs for policy-assignment output when available.
    output_columns = [
        "transaction_id",
        "customer_id",
        "merchant_id",
    ...
    if args.model:
        ...
        # Fail early if the evaluation matrix does not match the trained model.
        if model_type == "catboost":
            ...
            if model_feature_count != len(feature_columns):
                raise RuntimeError(
                    f"CatBoost feature mismatch: model expects "
                    f"{model_feature_count} features, but evaluator built "
                    f"{len(feature_columns)}. Expected training schema: "
                    f"{MODEL_FEATURES}"
                )

            if model_feature_names and model_feature_names != feature_columns:
                raise RuntimeError(
                    "CatBoost feature-order/name mismatch.\n"
                    f"Model:      {model_feature_names}\n"
                    f"Evaluator:  {feature_columns}"
                )
