#!/usr/bin/env python3
"""
RecoveryOS - Step 2: dataset audit + leakage-safe train/val/test preparation.

Designed for the 20M-row Razorpay RecoveryOS synthetic dataset generated by
 generate_static_v2.py.

What this script does
---------------------
1. Audits schema, row count, nulls, cardinality, recovery rates, and action mix.
2. Checks oracle-action dominance and action-level performance.
3. Creates a customer-level 80/10/10 split using a deterministic hash of customer_id.
4. Creates model-ready parquet files while EXCLUDING oracle/counterfactual columns
   from the model feature set.
5. Preserves observed intervention + observed outcome for offline policy evaluation.
6. Adds derived features that are safe to use before observing the action/outcome.
...
Example
-------
    python prepare_recovery_data.py \
      --input razorpay_recovery_v2_20m.parquet \
      --outdir recovery_prepared
"""
