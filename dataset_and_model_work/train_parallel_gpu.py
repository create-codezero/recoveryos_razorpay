#!/usr/bin/env python3
"""
RecoveryOS - Parallel GPU Model Training
=========================================

Trains 3 independent models concurrently, one per GPU:

    GPU 0 -> CatBoostClassifier
    GPU 1 -> XGBoostClassifier
    GPU 2 -> LightGBM LGBMClassifier

GPU 3 is intentionally left free for an LLM / experimentation.

The three models learn:

    P(recovered | observable context, observed intervention)

The oracle/counterfactual columns from V2 are NEVER used as model features.
They are reserved for offline policy evaluation after training.

Windows-safe:
    multiprocessing start method = "spawn"

Expected input files from prepare_recovery_data.py:

    recovery_prepared/
        recovery_train.parquet
        recovery_validation.parquet
        recovery_test.parquet

Recommended first run:
    python train_parallel_gpu.py --data-dir recovery_prepared --allow-cpu-fallback

Optional:
    python train_parallel_gpu.py --data-dir recovery_prepared --threads 16 --allow-cpu-fallback
...
NOTE:
- XGBoost GPU uses device="cuda:<ordinal>" + tree_method="hist" in current
  XGBoost versions.
- CatBoost uses task_type="GPU", devices="<ordinal>".
- LightGBM GPU configuration differs by installation. This script first
  tries device_type="cuda"; if that build does not support CUDA it can
  fall back to device_type="gpu". See --allow-cpu-fallback.
"""
